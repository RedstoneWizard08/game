...
#ifndef __RENDER_IMGUI_CPP
#define __RENDER_IMGUI_CPP
...
void Wrap_SDL_SetClipboardText(void*, const char* s) {
...
RenderImGuiImpl::RenderImGuiImpl()
    : shader(vertex_shader, fragment_shader), timestamp(SDL_GetTicks()),
      mouse_button_pressed(false), enter_pressed(false) {
    IMGUI_CHECKVERSION();
    ImGui::CreateContext();
...
RenderImGuiImpl::~RenderImGuiImpl() { ImGui::DestroyContext(); }
...
void RenderImGui::Render(SDL_Window* window, bool reset) {
    ...
    io.MouseDown[0] = self->mouse_button_pressed ||
    ...
    ImGui::Text("Hello %d %d %d", io.WantCaptureMouse, io.WantCaptureKeyboard,
                io.WantTextInput);
    ...
    ImGui::PushStyleColor(ImGuiCol_WindowBg,
                          ImVec4(0, 0, 0, io.WantTextInput ? 1.0f : 0.7f));
    ...
    ImGui::BeginChild(
        "output-area",
        ImVec2(ImGui::GetContentRegionAvail().x, chat_window_height - 50),
        false, 0);
    ...
    if (ImGui::InputText("Send", input, 100,
                         ImGuiInputTextFlags_EnterReturnsTrue)) {
        if (input[0] != '\0') {
            chat_lines.push_back(input);
        }
    ...
    glVertexAttribPointer(self->loc_a_xy, 2, GL_FLOAT, GL_FALSE,
                          sizeof(ImDrawVert),
                          reinterpret_cast<GLvoid*>(offsetof(ImDrawVert, pos)));
    glVertexAttribPointer(self->loc_a_uv, 2, GL_FLOAT, GL_FALSE,
                          sizeof(ImDrawVert),
                          reinterpret_cast<GLvoid*>(offsetof(ImDrawVert, uv)));
    glVertexAttribPointer(self->loc_a_rgba, 4, GL_UNSIGNED_BYTE, GL_TRUE,
                          sizeof(ImDrawVert),
