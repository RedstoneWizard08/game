...
#ifndef __RENDER_SURFACE_CPP
#define __RENDER_SURFACE_CPP
...
RenderSurface::RenderSurface(SDL_Surface* surface)
    : self(new RenderSurfaceImpl(surface)) {}
...
void RenderSurface::Render(SDL_Window* window, bool reset) {
    ...
    glBindBuffer(GL_ARRAY_BUFFER, self->vbo_pos.id);
    glVertexAttribPointer(self->loc_a_position, 2, GL_FLOAT, GL_FALSE,
                          2 * sizeof(GLfloat), 0);
    glBindBuffer(GL_ARRAY_BUFFER, self->vbo_tex.id);
    glVertexAttribPointer(self->loc_a_texcoord, 2, GL_FLOAT, GL_FALSE,
                          2 * sizeof(GLfloat), 0);
