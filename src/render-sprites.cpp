...
#ifndef __RENDER_SPRITES_CPP
#define __RENDER_SPRITES_CPP
...
void RenderSprites::Render(SDL_Window* window, bool reset) {
    ...
    glBufferData(GL_ELEMENT_ARRAY_BUFFER,
                 sizeof(GLushort) * self->indices.size(), self->indices.data(),
                 GL_DYNAMIC_DRAW);

    glBindBuffer(GL_ARRAY_BUFFER, self->vbo_attributes.id);
    glBufferData(GL_ARRAY_BUFFER, sizeof(Attributes) * self->vertices.size(),
                 self->vertices.data(), GL_STREAM_DRAW);

    glVertexAttribPointer(
        self->loc_a_corner, 2, GL_FLOAT, GL_FALSE, sizeof(Attributes),
        reinterpret_cast<GLvoid*>(offsetof(Attributes, corner)));

    glVertexAttribPointer(
        self->loc_a_texcoord, 2, GL_FLOAT, GL_FALSE, sizeof(Attributes),
    ...
    glVertexAttribPointer(
        self->loc_a_rotation, 1, GL_FLOAT, GL_FALSE, sizeof(Attributes),
